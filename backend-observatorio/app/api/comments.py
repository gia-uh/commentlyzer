...
import requests
import re
from .utilswc import gen_word_cloud_data, parse_gen_colors
...
def comments_wc(id):
    ...
    words = gen_word_cloud_data(coms, WRODS_NUMBER)
    #wordspol = parse_gen_colors(coms)
    #words = [{"name": i,"value":wordspol[i]['total'],'wdata':wordspol[i]} for i,j in sorted(wordspol.items(),key=lambda x:x[1]['total'],reverse=True)]
